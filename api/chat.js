...
const PRODUCT_KNOWLEDGE = {
  ...
  packages: [
    { name: '创业套餐', price: '¥15,000', description: '适合个人创业者和初创企业' },
    { name: '成长套餐', price: '¥35,000', description: '适合小型企业，提供全面安全服务' }
  ],
  company: {
    name: '安盾科技',
    contact: { email: 'contact@andun.io', discord: 'https://discord.com/invite/clawd' }
  }
};
...
module.exports = async (req, res) => {
  ...
  if (req.method !== 'POST') {
    // GET request - return session status if sessionId provided
    ...
    if (sessionId) {
      ...
      if (!session) {
        res.json({ success: true, exists: false, messageCount: 0, remainingMessages: CONFIG.maxMessages });
        return;
      }
      res.json({
        success: true,
